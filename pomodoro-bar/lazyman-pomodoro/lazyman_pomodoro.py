    ...
    try:
        ...
        while sec_remaining > 0:
            ...
            stdscr.addstr(2, 0, "CTRL + C to " + ("Pause" if is_working else "Skip") + \
                    "           ", curses.A_BOLD)
            os.system("echo " + str(math.floor(session % 8 / 2) + 1) + ("W" if is_working else "B") + sec_to_hhmmss(sec_remaining) + " > /tmp/.volume-pipe")
            stdscr.refresh()
            time.sleep(1)
            sec_remaining -= 1
        # os.system(f"echo . | $(which xnotify) -g 3840x2160 -s 2")
        cur_session = "Break" if is_working else "Work"
#             os.system("echo 'Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break! Break!' > /tmp/.volume-pipe")
        ...
        if session % 8 in [1, 3, 5, 7]:
            os.system("echo '-PAUSE-' > /tmp/.volume-pipe")
            stdscr.addstr(1, 0, "Time Left: " + sec_to_hhmmss(sec_remaining) + \
                    ' (Pause)', curses.A_BOLD)
...
def main(stdscr):
    ...
    os.system("echo '-START-' > /tmp/.volume-pipe")
    try:
